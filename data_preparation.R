...
######### This script is for importing, cleaning, and merging the raw data
...
# 3 A NA NA NA data has a note 'not sure which plot this is', not merging it into vitaldata
#Similarly not sure what TROR E 4 is, not merging into vitaldata
...
#To deal with NA problems in scaling abundance values, I will change NAs to 0
...
seed_fill<-read_csv("Data/seed_fill_towers_oecologia_2022.csv") %>%
...
#Don't want to exponentiate cases where there are no seeds. Want 0.
#Solution - filter to things that have viable seeds. Calculate exp(mean(log)), merge back, everything else is 0.
